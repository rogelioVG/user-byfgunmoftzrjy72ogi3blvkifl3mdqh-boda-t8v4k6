  ...
  <section class="hud">
    ...
    <div class="instructions" id="instructions">
      <div class="instructions-header">
        ...
        <button class="close-btn" id="close-instructions" aria-label="Cerrar instrucciones">×</button>
      </div>
      <ul class="actions">
        <li><strong>W/A/S/D</strong> Mover jugador (2 pasos por dirección)</li>
        <li><strong>Mouse</strong> Rotar cámara</li>
        <li><strong>Rueda</strong> Acercar/Alejar</li>
      </ul>
    </div>
    <div class="spacing-control">
      <label for="dancer-spacing">Distancia entre bailarines:</label>
      <input type="range" id="dancer-spacing" min="0.5" max="3.5" step="0.1" value="2.5">
...
const createDancer = (age = 'adult', skinTone = 0xffdbac, clothingColor = 0x4a90e2) => {
  ...
  dancerGroup.userData = {
    ...
    body,
    head,
    phase: 0, // Todos con la misma fase (sin variación)
    speed: 1.0, // Todos con la misma velocidad
    moveAngle: 0, // Sin variación
    moveSpeed: 0, // Sin movimiento individual
    baseRadius: 0 // No usado
  }
...
// Espaciado entre bailarines (aumentado)
let rowSpacing = 3.5 // Distancia entre filas (aumentado de 2.5)
let dancerSpacing = 2.5 // Distancia entre bailarines en la misma fila (aumentado de 1.5)
let startX = -(dancersPerRow - 1) * dancerSpacing / 2 // Posición inicial X
...
for (let row = 0; row < rows; row++) {
  for (let col = 0; col < dancersPerRow; col++) {
    ...
    dancer.userData.baseX = x // Guardar posición base X
    dancer.userData.baseZ = z // Guardar posición base Z
...
// Controles del jugador con WASD (step-based)
const playerVelocity = new THREE.Vector3()
const playerSpeed = 0.2
const keys = {}
const stepQueue = [] // Cola de pasos pendientes
let currentSteps = { w: 0, s: 0, a: 0, d: 0 } // Contador de pasos por dirección
const stepsPerDirection = 2 // Cada dirección requiere 2 pasos

window.addEventListener('keydown', (e) => {
  const key = e.key.toLowerCase()
  if (!keys[key] && (key === 'w' || key === 's' || key === 'a' || key === 'd')) {
    keys[key] = true
    currentSteps[key]++
    
    // Si se completan 2 pasos en una dirección, agregar movimiento
    if (currentSteps[key] >= stepsPerDirection) {
      stepQueue.push(key)
      currentSteps[key] = 0 // Resetear contador
    }
  }
})
...
// Actualizar movimiento del jugador con WASD (step-based)
const updatePlayerMovement = (delta) => {
  if (gameState.gameOver) return
  
  // Procesar pasos de la cola
  if (stepQueue.length > 0) {
    const direction = stepQueue.shift()
    const stepDistance = 0.5 // Distancia de cada paso
    ...
    if (distanceFromCenter < maxDistance) {
      // Rotar hacia la dirección de movimiento antes de mover
      if (direction === 'w') player.rotation.y = 0
      else if (direction === 's') player.rotation.y = Math.PI
      else if (direction === 'a') player.rotation.y = Math.PI / 2
      else if (direction === 'd') player.rotation.y = -Math.PI / 2
...
const animateSynchronizedDance = (dancer, time, delta, isPlayer = false) => {
  ...
  if (!isPlayer) {
    ...
    // Todos los bailarines se mueven igual (sin variación)
...
// Control de espaciado entre bailarines
const spacingSlider = document.getElementById('dancer-spacing')
const spacingValue = document.getElementById('spacing-value')

spacingSlider.addEventListener('input', (e) => {
  const newSpacing = parseFloat(e.target.value)
  spacingValue.textContent = newSpacing.toFixed(1)
  dancerSpacing = newSpacing
  
  // Actualizar posiciones de todos los bailarines
  dancers.forEach((dancer, index) => {
    const row = Math.floor(index / dancersPerRow)
    const col = index % dancersPerRow
    const newX = startX + col * dancerSpacing
    const newZ = (row - 1) * rowSpacing
    
    dancer.userData.baseX = newX
    dancer.userData.baseZ = newZ
    dancer.position.x = newX
    dancer.position.z = newZ
  })
  
  // Actualizar posición del jugador
  const playerCol = 4.5
  const playerRow = 1
  startX = -(dancersPerRow - 1) * dancerSpacing / 2
  player.position.x = startX + playerCol * dancerSpacing
  player.position.z = (playerRow - 1) * rowSpacing
})
